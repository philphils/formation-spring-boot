...
- **Ce qui se passe** : Le JSON est converti en objet Java et passé en paramètre de la méthode
...
**Ce qui se passe** : La méthode retourne un objet `User` convertit en JSON par Jackson
...
- **Pour les petites APIs** : Utiliser directement l'Entity avec annotations Jacskon (@JsonBackReference, @JsonManagedReference et autre cf. suite)
